...
public class Sender extends JButton implements Runnable, PausableProcess {
        ...
        while (!terminated) {
            ...
            try {
                ...
                try {
                    ...
                    System.out.println(this.devNum + " SENT MESSAGE{" + message + "}!!");
    ...
    private Byte[] getNewEncryptedMessage() {
        ...
        int prevSize = 2;
        System.arraycopy(senPhArr, 0, maxSmsC, prevSize, senPhArr.length + prevSize - prevSize); // address value
        ...
        if (recPhArr.length > 10)
            throw new InvalidGsmMessageFormatException("Receiver phone cant be more than 20 digits");

        prevSize += 2;
        System.arraycopy(recPhArr, 0, maxTPDU, prevSize, recPhArr.length + prevSize - prevSize); // tp-da address value
        ...
        System.arraycopy(msgBytes, 0, maxTPDU, prevSize, msgBytes.length + prevSize - prevSize); // tp ud
    ...
    private byte[] getByteMessage() {
        if (message.length() > 140)
            throw new InvalidGsmMessageFormatException("Message is exceeding 140 characters. Unable to send.");
    ...
    private byte[] toSemiOctet(String phone) {
        ...
        if (senPh.length() > 510)
            throw new InvalidGsmMessageFormatException("Number length exceeds 510, impossible to parse");
